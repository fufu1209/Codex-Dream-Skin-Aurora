...
- Windows 换肤升级为 Aurora Atelier：采用冰青、珊瑚、雾紫与奶油玻璃质感，重新设计首页 Hero、建议卡片、侧栏、编辑器和消息容器。
- 增加响应式尺寸调整与更克制的悬浮动效，保留辅助透明窗口的清理逻辑。
...
- Windows 现在使用可校验的 `assets/theme.json` 主题模型，支持主题名称、品牌文案、配色和背景图注入，不再把这些内容写死在渲染器中。
- Windows injector 版本与 macOS 主题协议对齐到 1.1.2，并增加自定义主题目录与 payload 占位符校验。
