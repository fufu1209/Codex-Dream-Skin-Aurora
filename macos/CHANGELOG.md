...
- 全面更新为 Aurora Atelier 视觉系统：深色墨蓝、冰青、珊瑚和雾紫构成新的主色层级。
- 重做首页 Hero、建议卡片、侧栏选中态、编辑器、消息容器、玻璃质感和响应式布局。
- 统一 macOS / Windows 的设计语言，同时保留两端各自的浅色与深色壳层适配。

### 改进

- 将 macOS doctor 测试改为使用隔离的临时 HOME，避免开发机残留的半成品主题状态导致仓库测试误报。
- 增加仓库根目录 MIT 许可证与 GitHub Actions 静态校验，降低二次开发和开源发布门槛。
